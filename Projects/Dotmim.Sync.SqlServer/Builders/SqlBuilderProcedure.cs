...
namespace Dotmim.Sync.SqlServer.Builders
{
    public class SqlBuilderProcedure : IDbBuilderProcedureHelper
    {
        ...
        private SqlCommand BuildBulkDeleteCommand()
        {
            ...
            {
                var cc = ObjectNameParser.Create(c.ColumnName);
                ...
                var quotedColumnType = ObjectNameParser.Create(sqlDbTypeString, "[", "]").FullQuotedString;
            ...
            {
                var columnName = ObjectNameParser.Create(c.ColumnName);
        ...
        // Bulk Insert command
        //------------------------------------------------------------------
        private SqlCommand BuildBulkUpsertCommand()
        {
            SqlCommand sqlCommand = new SqlCommand();
        ...
        public void CreateBulkInsert()
        {
            ...
            CreateProcedureCommand(BuildBulkUpsertCommand, commandName);
        }
        public string CreateBulkInsertScriptText()
        {
            var commandName = this.sqlObjectNames.GetCommandName(DbCommandType.BulkInsertRows);
            return CreateProcedureCommandScriptText(BuildBulkUpsertCommand, commandName);
        }
            ...
            if (withFilter && this.Filters != null && this.Filters.Count > 0)
            {
                foreach (var c in this.Filters)
                {
                    ...
                    else
                    {
                        ...
                        var columnFilterName = ObjectNameParser.Create(c.ColumnName, "[", "]");
                        SqlParameter sqlParamFilter = new SqlParameter($"@{columnFilterName.FullUnquotedString}", sqlDbType);
                        sqlCommand.Parameters.Add(sqlParamFilter);
                    }
                }
            ...
            if (this.Filters != null && this.Filters.Count > 0)
            {
                ...
                foreach (var c in this.Filters)
                {
                    ...
                    if (!c.IsVirtual)
                    {
                        ...
                        unquotedColumnName = ObjectNameParser.Create(columnFilter.ColumnName).FullUnquotedString;
                    }
                    else
                    {
                        unquotedColumnName = ObjectNameParser.Create(c.ColumnName).FullUnquotedString;
                    }
